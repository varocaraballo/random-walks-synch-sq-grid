...
broad_casts = np.array([(k, broadcast_experiment(G, startingPositions, k, n*n, protocol, 16*n*n if protocol != drones_team.PR_DETERMNISTIC else min(k*n, 2*n*n), 16*n*n)) for k in range(2,n*n+1) if k <= l])
...
f = open("broadcast_{0}_{1}_{2}".format(n,n, 'random' if protocol == drones_team.PR_RANDOM else ('pseudo_random' if protocol == drones_team.PR_PSEUDO_RANDOM else 'deterministic')),"wb")
